...
public class SharedListDetailDTO {
	...
	public SharedListDetailDTO(MovieList share, User user) {
		...
		this.sharedTo = share.getSharedTo().stream().map(UserSearchDTO::new).toList();
		this.direction = owner ? "sent" : "received";

		List<User> participants = Stream.concat(Stream.of(share.getUser()), share.getSharedTo().stream()).distinct()
				.filter(u -> !u.getId().equals(user.getId()))
				.toList();

		this.ratings = participants.stream()
				.flatMap(u -> share.getMovies().stream().map(movie -> new UsersRatingsDTO(u, movie))).toList();
	}
}